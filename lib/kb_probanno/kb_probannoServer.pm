package kb_probanno::kb_probannoServer;
...
sub call_method {
    ...
    my $ctx = kb_probanno::kb_probannoServerContext->new($self->{loggers}->{userlog},
...
{
    # Service kb_probanno requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for kb_probanno but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = kb_probanno::kb_probannoServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service kb_probanno.");
    }
...
package kb_probanno::kb_probannoServerContext;

use strict;

=head1 NAME

kb_probanno::kb_probannoServerContext
...
package kb_probanno::kb_probannoServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = kb_probanno::kb_probannoServer->new(
