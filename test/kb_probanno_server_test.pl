...
my $config = new Config::Simple($config_file)->get_block('kb_probanno');
...
$kb_probanno::kb_probannoServer::CallContext = $ctx;
my $impl = new kb_probanno::kb_probannoImpl();

sub get_ws_name {
    if (!defined($ws_name)) {
        my $suffix = int(time * 1000);
        $ws_name = 'test_kb_probanno_' . $suffix;
...

{
    ...
    sub provenance {
        my($self) = @_;
        return [{'service' => 'kb_probanno', 'method' => 'please_never_use_it_in_production', 'method_params' => []}];
    }
    sub authenticated {
        return 1;
    }
